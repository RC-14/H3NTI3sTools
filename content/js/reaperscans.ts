...
const getChapterButtons = () => {
	if (!chapterButtons.previous) chapterButtons.previous = qs('nav > div:first-child > a:first-child');
	if (!chapterButtons.next) chapterButtons.next = qs('nav > div:last-child > a:last-child');
};
