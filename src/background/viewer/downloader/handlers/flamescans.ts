...
const getSeriesUrlFromChapterHtml = (html: string) => {
	...
	if (url === undefined) throw new Error(`Couldn't parse flamescans chapter html for series url.`);
...
const chapterNumberFromTitle = (title: string) => {
	...
	if (afterChapter === undefined) throw new Error(`Couldn't parse flamescans chapter number from title. (doesn't contain "chapter")`);

	const beforeDash = afterChapter.split('-')[0];
	if (beforeDash === undefined) throw new Error(`Couldn't parse flamescans chapter number from title. (doesn't contain "-")`);
...
const addToCollection = (chapterUrl: string, chapterTitle: string, seriesName: string, seriesUrl: string) => new Promise<void>(async (resolve, reject) => {
	...
	request.addEventListener('error', (event) => {
		db.close();
		reject(new Error(`[flamescans] Couldn't update collection ("${parsedCollection.data.name}"): ${request.error}`));
	});
